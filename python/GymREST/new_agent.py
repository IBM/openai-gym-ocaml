...
if __name__ == '__main__':
    ...
    # Set up client
    remote_base = 'http://rl_server:5000'
    ...
    for i in range(episode_count):
        ...
        for j in range(max_steps):
            ...
            # print('Action taken: %d\nReward gained: %d' % (action, reward))
            ob, reward, done, _ = client.env_step(instance_id, action, render=True)
            try:
                val = float(reward)
                cum_reward += reward
            except ValueError:
                pass  # Reward should be a number - if it is not, we gracefully degrade and move on.

            if done:
                break

        print('Cumulative reward gained in episode %d is %d' % (i, cum_reward))
